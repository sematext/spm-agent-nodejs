        ...
        var timerId = setInterval(function () {
          ...
          if (self.isLinux) {
            linux.vmstat(function (err, vmstat) {
              ...
              metrics['collectd-swap-used'] = vmstat.swapd
              ...
              linux.vmstatS(function (err, vmstats) {
                ...
                metrics['collectd-mem-free'] = vmstats.memory.free
                metrics['collectd-mem-buff'] = vmstats.memory.buffer
                metrics['collectd-mem-inactive'] = vmstats.memory.inactive
                metrics['collectd-mem-active'] = vmstats.memory.active
                metrics['collectd-mem-used'] = vmstats.memory.used
                var cpuTotal = 0
                // we get summary of all cpu's so in collectd we send cpu-0
                var i = 0
                cpuProperties.forEach(function (property) {
                  cpuLastValues[i][property] = vmstats.cpu [property] - cpuLastValues[i][property]
